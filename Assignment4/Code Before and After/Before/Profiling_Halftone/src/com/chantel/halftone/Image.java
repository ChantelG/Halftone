...
public class Image 
{
	...
	// Store all of the various colours so that they can be referenced rather than instantiated several times unnecessarily
	private HashMap<Integer, Color> greyVarients = new HashMap<Integer, Color>();
		...
		for (int i = 0; i < height; i++) 
		{
			pixels.add(new ArrayList<Integer>());
		      for (int j = 0; j < width; j++) 
		      {
		        ...
		        int grey = (int)((pixelRGB.getRed()*0.2126)+(pixelRGB.getGreen()*0.7152)+(pixelRGB.getBlue()*0.0722));
		        
		        /* If the grey color has not been encountered before, put it into the greyVarients hashmap so it can be reused if the same
